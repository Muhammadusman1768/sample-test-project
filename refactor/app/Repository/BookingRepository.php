...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if ($jobs) {
            $emergencyJobs = $jobs->where('immediate', 'yes');
            $normalJobs = $jobs->where('immediate', 'no');
        }
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        return ['emergencyJobs' => $emergencyJobs, 'normalJobs' => $normalJobs, 'jobs' => $jobs, 'cuser' => $user, 'usertype' => $usertype, 'numpages' => $numPages ?? 0, 'pagenum' => $page];
    }
    ...
    public function store(User $user, Request $request)
    {
        ...
        $request->validate([
            ...
            'customer_phone_type' => 'in:on',
            'customer_physical_type' => 'in:on',
            'duration' => 'required',
        ]);

        $data = $request->all();
        $data['customer_phone_type'] = $request->has('customer_phone_type') ? 'yes' : 'no';
        $data['customer_physical_type'] = $request->has('customer_physical_type') ? 'yes' : 'no';
        ...
        if ($data['immediate'] == 'yes') {
            ...
            $data['due'] = $dueCarbon->format('Y-m-d H:i:s');
            if ($dueCarbon->isPast()) {
                return response()->json(['status' => 'fail', 'message' => "Can't create booking in the past"], 400);
            }
        }

        // Determine 'gender' and 'certified' fields
        $jobFor = $data['job_for'];
        ...
        foreach ($certifiedOptions as $option => $certifiedType) {
            if (in_array($option, $jobFor)) {
                $data['certified'] = $certifiedType;
                break;
            }
        }
        ...
        // Determine 'job_type' based on 'consumer_type'
        ...
        return response()->json(['status' => 'success', 'id' => $job->id], 201);
    }
    ...
    public function storeJobEmail(array $data)
    {
        ...
        if (isset($data['address'])) {
            $user = $job->user;
            $job->address = isset($data['address']) ? $data['address'] : $user->userMeta->address;
            $job->instructions = isset($data['instructions']) ? $data['instructions'] : $user->userMeta->instructions;
            $job->town = isset($data['town']) ? $data['town'] : $user->userMeta->city;
        }
        ...
        Mail::to($email)->send(new JobCreatedMail($name, $subject, $sendData));
    ...
    public function jobEnd($jobId, $userId)
    {
        ...
        $this->sendSessionEmail($user, $job, $interval, 'faktura');
        Event::fire(new SessionEnded($job, ($userId == $job->user_id) ? $tr->user_id : $job->user_id));

        $translatorUser = $tr->user;
        $this->sendSessionEmail($translatorUser, $job, $interval, 'lön');
    ...
    private function sendSessionEmail($user, $job, $sessionTime, $forText)
    {
    ...
    public function getPotentialJobIdsWithUserId(int $user_id)
    {
        ...
        $jobIds = Job::getJobs($user_id, $job_type, 'pending', $userLanguages, $gender, $translator_level);

        $filteredJobIds = $this->filterJobsByTown($user_id, $jobIds);
    ...
    private function filterJobsByTown(int $user_id, $jobIds)
    {
        $filteredJobIds = [];
     ...
     */
    public function sendNotificationTranslator(Job $job, $data = [], $exclude_user_id)
    {
        $suitableTranslators = $this->getSuitableTranslators($exclude_user_id, $data);

        $notificationType = 'suitable_job';
        $msgContents = $this->getMessageContents($data);
        $msgText = ["en" => $msgContents];

        Log::info('Push send for job ' . $job->id, [$suitableTranslators, $msgText, $data]);
    ...
    private function getSuitableTranslators($exclude_user_id, $data)
    {
        $regularTranslators = collect([]);
        $delayedTranslators = collect([]);

        $users = User::where('user_type', 'translator')
            ->where('status', '1')
                ...
                if ($data['job_id'] === $oneJob->id) {
                    ...
                    if ($jobForTranslator === 'SpecificJob') {
                        ...
                        if ($jobChecker !== 'userCanNotAcceptJob') {
                            $translatorArray = $this->isNeedToDelayPush($oneUser->id) ? $delayedTranslators : $regularTranslators;
                            $translatorArray->push($oneUser);
                        }
                    }
                }
            }
        ...
        return [
            'regular' => $regularTranslators,
            'delayed' => $delayedTranslators,
        ];
    }
    ...
    private function getMessageContents($data)
    {
        ...
        return "Ny akutbokning för $language tolk $duration min";
    }
    /**
     ...
     * @return int
     */
    public function sendSMSNotificationToTranslator(Job $job, SmsHandler $smsHandler)
    {
        ...
        $phoneJobMessageTemplate = __('sms.phone_job', ['date' => $date, 'time' => $time, 'duration' => $duration, 'jobId' => $jobId]);
        $physicalJobMessageTemplate = __('sms.physical_job', ['date' => $date, 'time' => $time, 'town' => $city, 'duration' => $duration, 'jobId' => $jobId]);

        // Analyze whether it's a phone or physical job; if both, default to phone
        ...
        foreach ($translators as $translator) {
            ...
            $status = $smsHandler->send(env('SMS_NUMBER'), $translator->mobile, $message);
     ...
     * @return bool
     */
    public function isNeedToDelayPush($user_id, UserMetaService $userMetaService)
    {
        if (DateTimeHelper::isNightTime()) {
            $not_get_nighttime = $userMetaService->getUserMeta($user_id, 'not_get_nighttime');
     ...
     * @return bool
     */
    public function isNeedToSendPush($user_id, UserMetaService $userMetaService)
    {
        $not_get_notification = $userMetaService->getUserMeta($user_id, 'not_get_notification');
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        switch ($jobType) {
            ...
            case 'unpaid':
                ...
                break;
            default:
                $translatorType = null; // Set a default value or handle the case differently if needed
                break;
        }
    ...
    public function updateJob($id, $data, $cuser)
    {
        ...
        $this->logJobUpdate($cuser, $job, $logData);
...
// Separate methods for specific logic:
    ...
    private function logJobUpdate($cuser, $job, $logData)
    {
